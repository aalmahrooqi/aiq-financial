...
```{note}
**Hosted Endpoint Availability:** Nemotron Super (`nvidia/nemotron-3-super-120b-a12b`) and Nemotron Ultra (`nvidia/nemotron-3-ultra-550b-a55b`) are compatible and tested with AIQ, but their hosted endpoints can have limited availability during high demand (HTTP 429 or 503 responses). The default configs use Nemotron Super for intent classification, shallow research, and deep-research writing, and Nemotron Ultra for the other deep-research roles. For production deployments requiring consistent throughput, refer to the model-specific [self-hosting guidance](../resources/troubleshooting.md#nemotron-super-and-ultra--hosted-endpoint-availability).
```
